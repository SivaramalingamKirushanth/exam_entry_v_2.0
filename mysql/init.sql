...
BEGIN
    ...
    CASE
        ...
        WHEN mr.s_id IS NOT NULL AND mr.reference = '' AND mr.status = '' AND NOW() < b.payment_end THEN 'payment pending'
...
BEGIN
    ...
    CASE
        ...
        WHEN rr.s_id IS NOT NULL AND rr.reference = '' AND rr.status = '' AND NOW() < b.payment_end THEN 'payment pending'
...
/*!50003 SET sql_mode              = 'NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE PROCEDURE `UpsertPayment`(IN p_type VARCHAR(250), IN p_amount DECIMAL(18,2))
BEGIN
